...
export async function registerSubscription(env, session, body, config) {
  ...
  const owner=await env.DB.prepare('SELECT user_id FROM push_subscriptions WHERE endpoint_hash=?1').bind(sub.endpointHash).first();
  if (owner && owner.user_id!==session.user.id) throw pushError('subscription_owned_elsewhere',409);
  const row=await env.DB.prepare(`INSERT INTO push_subscriptions
    ...
    ON CONFLICT(endpoint_hash) DO UPDATE SET session_id=excluded.session_id,p256dh=excluded.p256dh,auth=excluded.auth,
    ...
    .bind(uuid(),session.user.id,session.session.id,sub.endpointHash,sub.endpoint,sub.p256dh,sub.auth,config.fingerprint,sub.expiration,now).first();
...
export async function sendDelivery(env, deliveryId, config, {fetchImpl=fetch,now=Date.now()}={}) {
  ...
  const row=await env.DB.prepare(`SELECT d.*,n.user_id,n.saved_id,n.type,n.payload,n.expires_at,n.id AS notification_id,
    ...
    WHERE d.id=?1 AND s.user_id=n.user_id AND sess.user_id=s.user_id AND julianday(sess.expires_at)>julianday('now')`).bind(deliveryId).first();
